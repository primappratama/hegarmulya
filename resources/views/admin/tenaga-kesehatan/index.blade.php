<x-app-layout>
    <x-slot name="header"><span>Tenaga Kesehatan</span></x-slot>
    ...
    <div x-data="{ showAdd: false }" class="mb-5">
        <button @click="showAdd = !showAdd" class="plist-toggle">
            ...
            <span x-text="showAdd ? 'Tutup Form' : 'Tambah Tenaga'"></span>
        </button>
        <div x-show="showAdd" x-cloak x-transition class="plist-add-card mt-3">
            <form action="{{ route('admin.tenaga-kesehatan.store') }}" method="POST" class="grid grid-cols-3 gap-3">
                @csrf
                <div><label class="block text-xs font-medium mb-1">Jenis Tenaga</label><input type="text" name="jenis_tenaga" class="w-full rounded-lg px-3 py-2.5 text-sm"></div>
                ...
                <div><label class="block text-xs font-medium mb-1">Tahun</label><input type="number" name="tahun" value="{{ date('Y') }}" class="w-full rounded-lg px-3 py-2.5 text-sm"></div>
    ...
    @forelse ($items as $item)
        <div class="plist-row" x-data="{ editing: false }">
            <div class="plist-icon"><svg viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round"><path d="M12 8v8M8 12h8"/><circle cx="12" cy="12" r="9"/></svg></div>
            <template x-if="!editing"><div class="plist-body"><div class="plist-title">{{ $item->jenis_tenaga }}</div><div class="plist-sub">Tahun {{ $item->tahun }}</div></div></template>
            ...
            <template x-if="!editing">
                <div class="plist-actions">
                    ...
                    <form action="{{ route('admin.tenaga-kesehatan.destroy', $item) }}" method="POST" onsubmit="return confirm('Yakin hapus?')">@csrf @method('DELETE')<button type="submit" class="plist-btn danger"><svg viewBox="0 0 24 24" fill="none" stroke="#c0392b" stroke-width="2"><path d="M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2m3 0v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6"/></svg></button></form>
                </div>
            </template>
            <template x-if="editing">
                <form action="{{ route('admin.tenaga-kesehatan.update', $item) }}" method="POST" class="flex-1 flex items-center gap-2">
                    @csrf @method('PUT')
                    <input type="text" name="jenis_tenaga" value="{{ $item->jenis_tenaga }}" class="rounded-lg border-gray-200 shadow-sm text-sm flex-1">
