<x-app-layout>
    <x-slot name="header"><span>Data Sungai</span></x-slot>
    ...
    <div x-data="{ showAdd: false }" class="mb-5">
        <button @click="showAdd = !showAdd" class="plist-toggle">
            ...
            <span x-text="showAdd ? 'Tutup Form' : 'Tambah Sungai'"></span>
        </button>
        <div x-show="showAdd" x-cloak x-transition class="plist-add-card mt-3">
            <form action="{{ route('admin.sungai.store') }}" method="POST" class="space-y-3">
                @csrf
                <div><label class="block text-xs font-medium mb-1">Nama Sungai</label><input type="text" name="nama_sungai" class="w-full rounded-lg px-3 py-2.5 text-sm"></div>
    ...
    @forelse ($items as $item)
        <div class="plist-row" x-data="{ editing: false }">
            <div class="plist-icon"><svg viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round"><path d="M2 12c3-3 5 3 8 0s5 3 8 0 4 3 4 3M2 17c3-3 5 3 8 0s5 3 8 0 4 3 4 3"/></svg></div>
            <template x-if="!editing">
                <div class="plist-body"><div class="plist-title">{{ $item->nama_sungai }}</div><div class="plist-sub">{{ $item->keterangan }}</div></div>
            </template>
            <template x-if="!editing">
                <div class="plist-actions">
                    ...
                    <form action="{{ route('admin.sungai.destroy', $item) }}" method="POST" onsubmit="return confirm('Yakin hapus?')">@csrf @method('DELETE')<button type="submit" class="plist-btn danger"><svg viewBox="0 0 24 24" fill="none" stroke="#c0392b" stroke-width="2"><path d="M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2m3 0v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6"/></svg></button></form>
                </div>
            </template>
            <template x-if="editing">
                <form action="{{ route('admin.sungai.update', $item) }}" method="POST" class="flex-1 flex items-center gap-2">
                    @csrf @method('PUT')
                    <input type="text" name="nama_sungai" value="{{ $item->nama_sungai }}" class="rounded-lg border-gray-200 shadow-sm text-sm flex-1">
                    <input type="text" name="keterangan" value="{{ $item->keterangan }}" class="rounded-lg border-gray-200 shadow-sm text-sm flex-[2]">
    ...
    @empty
        <p class="text-gray-400 text-sm italic">Belum ada data sungai.</p>
    @endforelse
</x-app-layout>